...
<div id="mediamodal" class="modal large" tabindex="-1" role="dialog">
	...
	<div id="vvveb-builder">
				...
				<div id="top-panel">
					...
					<div class="btn-group mr-3" role="group">
					  <button class="btn btn-light" title="Export (Ctrl + E)" id="save-btn" data-vvveb-action="saveAjax" data-vvveb-shortcut="ctrl+e">
                        ...
                        <input type="hidden" name="pageon" id="pageon" value="<?php echo $_REQUEST["page"]; ?>">
				...
				<div class="btn-group" role="group" style="width: 100%">
                    ...
                    <button class="btn btn-success" data-vvveb-action="saveAjax">Save Page Content</button>
...
<div class="modal fade" id="message-modal" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <p class="modal-title text-primary">Caffeine Says</p>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true"><small><i class="la la-close"></i></small></span>
        </button>
      </div>
...
$(document).ready(function()
{
	...
	Vvveb.Builder.init('demo/narrow-jumbotron/index.php?page=<?php echo $_REQUEST["page"]; ?>', function() {
	...
	[
		{name:"narrow-jumbotron", title:"Edit Page",  url: "demo/narrow-jumbotron/index.php?page=<?php echo $_REQUEST["page"]; ?>", assets: ['']}
...
function setImgDat(inputTarget,img,alttext){
   //alert(inputTarget);
    ...
    window.parent.imgSucc();
}

function openFormBrowse(){
    console.log('TEST');

    $("#mediamodal .modal-body").html('<iframe src="../inc/form-passback.php" style="height:300px;width:100%; border: none"></iframe>')
    $("#mediamodal .modal-title").html('Forms');
    $("#mediamodal").modal();
...
function setFormDat(formname){
    //select-form
    $('input[name="select-form"]').val(formname);
    setTimeout(function(){
        $("#iframe1").contents().find('[data-system-form="'+formname+'"]').html("{form}"+formname+"{/form}");
    }, 100);

    $('input[name="select-form"]').focus();
}

function alertthesound(){
    parent.alertSoundSave();
}
</script>
</body>
</html>
