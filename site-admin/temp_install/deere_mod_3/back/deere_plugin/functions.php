...
class deereClass
{
    ...
    function updateSEO($oldval, $seoval) {
        if(file_exists('../../inc/harness.php')){
            include('../../inc/harness.php');
        }else{
            include('inc/harness.php');
        }
        $a = $data->query("UPDATE deere_equipment SET eq_image = REPLACE(eq_image, '$oldval', '$seoval')");
    ...
    function getProduct($prod, $cat)
    {
        ...
        $a = $data->query("SELECT * FROM deere_equipment WHERE title = '$prod'");
        $b = $a->fetch_array();
    ...
    function searchProduct($search)
    {
        ...
        $a = $data->query("SELECT * FROM deere_equipment WHERE title LIKE '%" . $search . "%' ");

//        return $a->num_rows;

        while ($b = $a->fetch_array())  {
            $prods[] =  array("prod" => $b["title"]);
    ...
    function getcats($row)
    {
        ...
        $a = $data->query("SELECT * FROM deere_equipment WHERE $row != '' AND active = 'true' GROUP BY $row");
        while ($b = $a->fetch_array()) {
    ...
    function getEquipmentProducts($cattype, $catname)
    {
        ...
        if ($catname != '') {
            ...
            $checkRun = $data->query("SELECT * FROM deere_equipment WHERE active = 'true' AND $cattype = '$catname'");
            $che = $checkRun->fetch_array();

            if ($che["$cattype1"] == '') {
                $a = $data->query("SELECT * FROM deere_equipment WHERE active = 'true' AND $cattype = '$catname'");
            } else {
                $a = $data->query("SELECT * FROM deere_equipment WHERE active = 'true' AND $cattype = '$catname' GROUP BY $cattype1");
            }
            ...
            $a = $data->query("SELECT * FROM deere_equipment WHERE active = 'true' GROUP BY parent_cat");
            while ($b = $a->fetch_array()) {
    ...
    function getBackLink($cattype, $catname)
    {
        ...
        //echo $cattype .' - '. $catname;
        $a = $data->query("SELECT * FROM deere_equipment WHERE $cattype = '$catname'");
        $b = $a->fetch_array();
    ...
    function getDeerePage($name){
        ...
        $a = $data->query("SELECT * FROM deere_pages WHERE id = '$name'");
        $b = $a->fetch_assoc();
        return $b;
    }
    ...
    function getEqCats($filter)
    {
        ...
        $a = $data->query("SELECT * FROM deere_pages WHERE active = 'true' $addSql ORDER BY page_name ASC") or die($data->error);
        while ($b = $a->fetch_array()) {
    ...
    function getPagesCat($line){
        ...
        $ars = array();
        $a = $data->query("SELECT * FROM deere_pages WHERE active = 'true' ORDER BY page_name ASC");
        while($b = $a->fetch_array()){
    ...
    function updateProduct($post){
        ...
        $data->query("UPDATE deere_equipment SET eq_image = '".$data->real_escape_string($post["cat_img"])."', price = '".$post["pricepoint"]."', dealer_price = '".$data->real_escape_string($post["msrp"])."', quick_links_active = '".$post["quick_links_active"]."', offers_active = '".$post["offers_active"]."', videos_active = '".$post["videos_active"]."', access_active = '".$post["access_active"]."', reviews_active = '".$post["reviews_active"]."', cta_text = '".$data->real_escape_string($post["cta-text"])."',extra_content = '".$data->real_escape_string($post["promoarea"])."' WHERE id = '".$post["proid"]."'");
    ...
    function checkPages(){
        ...
        $a = $data->query("SELECT * FROM deere_pages WHERE active = 'true'");
    ...
    function getSetupPages(){
        ...
        $url = 'http://caffeinerde.com/plugins/john_deere_equipment/pageprocess.php';
...
// use key 'http' even if you send the request to https://...
        ...
        if($theArs["status"] == 'Good'){
            ...
            for($i=0; $i < count($deereEquip); $i++){
                ...
                $g = $data->query("SELECT * FROM deere_pages WHERE page_name = '".$deereEquip[$i]["page_name"]."'");
                if($g->num_rows > 0){
                    ///DO NOT INSERT//
                }else{
                    $data->query("INSERT INTO deere_pages SET page_name = '".$data->real_escape_string($deereEquip[$i]["page_name"])."', page_title = '".$data->real_escape_string($deereEquip[$i]["page_title"])."', page_content = '".$data->real_escape_string(htmlspecialchars_decode($deereEquip[$i]["page_content"]))."', active = '".$data->real_escape_string($deereEquip[$i]["active"])."', created = '".$data->real_escape_string($deereEquip[$i]["created"])."', last_edit = '".$data->real_escape_string($deereEquip[$i]["last_edit"])."', last_user = '".$data->real_escape_string($deereEquip[$i]["last_user"])."', page_lock = '".$data->real_escape_string($deereEquip[$i]["page_lock"])."', page_type = '".$data->real_escape_string($deereEquip[$i]["page_type"])."', page_desc = '".$data->real_escape_string($deereEquip[$i]["page_desc"])."', page_js = '".$data->real_escape_string($deereEquip[$i]["page_js"])."', equipment_content = '".$data->real_escape_string(htmlspecialchars_decode($deereEquip[$i]["equipment_content"]))."', cat_type = '".$data->real_escape_string($deereEquip[$i]["cat_type"])."', cat_img = '".$data->real_escape_string($deereEquip[$i]["cat_img"])."'")or die($data->error);
                }
            }
    ...
    function checkUpdates(){
        ...
        $a = $data->query("SELECT * FROM beans WHERE bean_folder = 'deere_plugin'");
        $b = $a->fetch_array();


        $url = 'http://caffeinerde.com/plugins/john_deere_equipment/updates.php';
        $key = 'IBN72234957UTYGALAXY!@';
        $password = 'YELLOANDGREENMAKEBLUE434';
