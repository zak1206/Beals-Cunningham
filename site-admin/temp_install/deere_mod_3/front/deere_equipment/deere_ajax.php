...
if($act == 'quickview'){
    ...
    if($prodId != null){
        ...
        if($equiptype == 'kuhn') {
            $a = $data->query("SELECT * FROM kuhn_equipment WHERE id = '$prodId'");
        }
        if($equiptype == 'woods') {
        ...
        if($equiptype == 'deere') {
            ...
            $imgdecode = json_decode($obj["eq_image"]);
//                var_dump($imgdecode);
            if(is_null($imgdecode)) {
                $image = 'img/equip_images/'.str_replace('"', '',$obj["eq_image"]);
            }else {
                $image = 'img/equip_images/' . $imgdecode[0];
