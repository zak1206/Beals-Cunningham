...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
  ...
  <lastmod>' . $updateDate . '</lastmod>
    ...
    function createForm($post){
        ...
        if($df->num_rows == 0){
            $data->query("INSERT INTO forms_data SET form_name = '$form_name', post_link = '$post_action', form_class = '$form_class', is_multi = '$multi', recipients = '$recipients', subject = '$subject', form_data = '$form_json', form_elems = '$formcontent', success_mess = '".$success_mess."'");
        }else{
...
$html .='</ul>';
    ...
    function updateForm($post){
        ...
        if($b["form_data"] == $post["form_json"]){
            ...
            $data->query("UPDATE forms_data SET post_link = '".$post["post_link"]."', subject = '".$post["subject"]."', form_class = '".$post["form_class"]."', is_multi = '".$post["multi"]."', success_mess = '".$data->real_escape_string($post["success_mess"])."', recipients = '".$post["recipients"]."' $formEdits WHERE form_name = '".$post["form_name"]."'")or die('Cannot update forms data');
            return 'success';
        }else{
    ...
    function readOrder($id){
        include('harness.php');
        $a = $data->query("SELECT * FROM custom_equipment_shop_orders WHERE id = '$id'");
        $b = $a->fetch_array();
        return $b;
    }
    ...
    function saveSmtpDets($post){
        ...
        $mail_password = $post["mail_password"];
        $mail_from = $post["mail_from"];
        $mail_from_email = $post["mail_from_email"];
        $mail_to = $post["mail_to"];
        $mail_email = $post["mail_email"];
        $mail_smtp_auth = $post["mail_smtp_auth"];
        $mail_debug = $post["mail_debug"];
        if(isset($mail_smtp_auth)){
            $smtpauth = 'true';
        }else{
            $smtpauth = '';
        }
        $data->query("UPDATE mail_settings SET host = '$mail_host', port = '$mail_port', user = '$mail_username', password = '$mail_password', from_name = '$mail_from', from_email = '$mail_from_email', smtp_auth = '$smtpauth' WHERE id = 1")or die($data->error);
    }
