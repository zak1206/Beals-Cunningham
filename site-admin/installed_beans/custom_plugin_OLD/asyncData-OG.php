...
if($act == 'openproduct'){
    ...
    $product = $kuhn->getProduct($_REQUEST["prod"]);
    //var_dump($product);
...
if($act == 'addneweq'){
    ...
    $parentCats = $kuhn->getcats('parent_cat');
    ...
    $cat_three = $kuhn->getcats('cat_three');
    ...
    $cat_four = $kuhn->getcats('cat_four');
    if(!empty($cat_four) ){
...
if($act == 'processEquip'){
    $kuhn->processSingleEquipment($_POST);
}
...
if($act == 'getprodcats'){
    ...
    if($_REQUEST["cattype"] != null && $_REQUEST["catname"] != null){
        ...
        $cats = $kuhn->getEquipmentProducts($_REQUEST["cattype"],$_REQUEST["catname"]);

        $categoryOut .= '<h4 style="text-align: center">'.$_REQUEST["catname"].'</h4>';


        $backLins = $kuhn->getBackLink($_REQUEST["cattype"],$_REQUEST["catname"]);
...
if($act == 'filtercustomcats'){
    $realCats = $kuhn->getEqCats($_REQUEST["filter"]);
...
if($act == 'pullupdates'){
    echo $kuhn->getUpdatePackage($_REQUEST["pack"]);

}

if($act == 'createbean'){
    $kuhn->createEqBean($_REQUEST["bean_id"]);
}
