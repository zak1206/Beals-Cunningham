...
if ($act == 'openproduct') {
    ...
    $product = $custom->getProduct($_REQUEST["prod"]);
    //var_dump($product);
...
if ($act == 'addneweq') {
    ...
    $parentCats = $custom->getcats('parent_cat');
    for ($o = 0; $o <= count($parentCats); $o++) {
        if ($parentCats[$o]["cat"] != '') {
            $parent .= '<option value="' . $parentCats[$o]["cat"] . '">' . $parentCats[$o]["cat"] . '</option>';
        }
    }
    ...
    $cat_one = $custom->getcats('cat_one');
    for ($p = 0; $p <= count($cat_one); $p++) {
        if ($cat_one[$p]["cat"] != '') {
            $sec .= '<option value="' . $cat_one[$p]["cat"] . '">' . $cat_one[$p]["cat"] . '</option>';
        }
    }
    ...
    $cat_two = $custom->getcats('cat_two');
    for ($q = 0; $q <= count($cat_two); $q++) {
        if ($cat_two[$q]["cat"] != '') {
            $third .= '<option value="' . $cat_two[$q]["cat"] . '">' . $cat_two[$q]["cat"] . '</option>';
        }
    }
    ...
    $cat_three = $custom->getcats('cat_three');
    for ($r = 0; $r <= count($cat_three); $r++) {
        if ($cat_three[$r]["cat"] != '') {
            $fourth .= '<option value="' . $cat_three[$r]["cat"] . '">' . $cat_three[$r]["cat"] . '</option>';
        }
    }
    ...
    $cat_four = $custom->getcats('cat_four');
    if (!empty($cat_four)) {
...
if ($act == 'processEquip') {
    ...
    $custom->processSingleEquipment($_POST);
}
...
if ($act == 'getprodcats') {
    ...
    if ($_REQUEST["cattype"] != null && $_REQUEST["catname"] != null) {
        ...
        $cats = $custom->getEquipmentProducts($_REQUEST["cattype"], $_REQUEST["catname"]);

        $categoryOut .= '<h4 style="text-align: center">' . $_REQUEST["catname"] . '</h4>';


        $backLins = $custom->getBackLink($_REQUEST["cattype"], $_REQUEST["catname"]);
...
if ($act == 'filtercustomcats') {
    ...
    $realCats = $custom->getEqCats($_REQUEST["filter"]);
...
if ($act == 'pullupdates') {
    ...
    echo $custom->getUpdatePackage($_REQUEST["pack"]);
}
...
if ($act == 'createbean') {
    ...
    $custom->createEqBean($_REQUEST["bean_id"]);
}
