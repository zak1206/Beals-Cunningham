...
class customClass
{
    ...
    function getProduct($prod, $cat)
    {
        ...
        $a = $data->query("SELECT * FROM custom_equipment WHERE title = '$prod'");
        $b = $a->fetch_array();
    ...
    function getcats($row)
    {
        ...
        $a = $data->query("SELECT * FROM custom_equipment WHERE $row != '' AND active = 'true' GROUP BY $row");
    ...
    function getEquipmentProducts($cattype, $catname)
    {
       if(file_exists('../../inc/harness.php')){
        ...
        if ($catname != '') {
            ...
            $checkRun = $data->query("SELECT * FROM custom_equipment WHERE active = 'true' AND $cattype = '$catname'");
            $che = $checkRun->fetch_array();

            if ($che["$cattype1"] == '') {
                $a = $data->query("SELECT * FROM custom_equipment WHERE active = 'true' AND $cattype = '$catname'");
            } else {
                $a = $data->query("SELECT * FROM custom_equipment WHERE active = 'true' AND $cattype = '$catname' GROUP BY $cattype1");
            }
            ...
            $a = $data->query("SELECT * FROM custom_equipment WHERE active = 'true' GROUP BY parent_cat");
            while ($b = $a->fetch_array()) {
    ...
    function getBackLink($cattype, $catname)
    {
        ...
        //echo $cattype .' - '. $catname;
        $a = $data->query("SELECT * FROM custom_equipment WHERE $cattype = '$catname'");
        $b = $a->fetch_array();
    ...
    function getDeerePage($name){
        ...
        $a = $data->query("SELECT * FROM custom_pages WHERE id = '$name'");
        $b = $a->fetch_assoc();
        return $b;
    }
    ...
    function getEqCats($filter)
    {
        ...
        $a = $data->query("SELECT * FROM custom_pages WHERE active = 'true' $addSql ORDER BY page_name ASC") or die($data->error);
        while ($b = $a->fetch_array()) {
    ...
    function getPagesCat($line){
        ...
        $ars = array();
        $a = $data->query("SELECT * FROM custom_pages WHERE active = 'true' ORDER BY page_name ASC");
        while($b = $a->fetch_array()){
            $ars[] = array("id"=>$b["id"],"page_name"=>$b["page_name"],"category_type"=>$b["cat_type"],"page_title"=>$b["page_title"],"active"=>$b["active"],"page_lock"=>$b["page_lock"],"page_template"=>$b["page_content"],"check_out"=>$b["check_out"],"check_out_date"=>$b["check_out_date"]);
        }
        return $ars;
    }
    ...
    function updateProduct($post){
        ...
        $img = $_POST["cat_img"];
        $data->query("UPDATE custom_equipment SET eq_image = '\[\"$img\"\]' , sales_price = '".$post["sales_price"]."', msrp = '".$data->real_escape_string($post["msrp"])."', parent_cat = '".$post["parent_cat"]."', cat_one = '".$post["offers_active"]."', description = '".$post["description"]."',features = '".$post["features"]."', title = '".$post["title"]."', sku = '".$post["sku"]."' WHERE id = '".$post["proid"]."'")or die($data->error);
    ...
    function checkPages(){
        ...
        $a = $data->query("SELECT * FROM custom_pages WHERE active = 'true'");
    ...
    function getSetupPages(){
        ...
        $url = 'http://caffeinerde.com/plugins/custom_equipment/pageprocess.php';
...
// use key 'http' even if you send the request to https://...
    ...
    function checkUpdates(){
        ...
        $a = $data->query("SELECT * FROM beans WHERE bean_folder = 'custom_plugin'");
        $b = $a->fetch_array();


        $url = 'http://caffeinerde.com/plugins/custom_equipment/updates.php';
        $key = 'IBN72234957UTYGALAXY!@';
        $password = 'YELLOANDGREENMAKEBLUE434';
