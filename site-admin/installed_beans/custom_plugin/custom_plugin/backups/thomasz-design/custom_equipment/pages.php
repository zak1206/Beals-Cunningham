    ...
    <div class="card" style="border:none;">
        <div class="row g-0">
            ...
            if ($obj["sales_price"] != 'Null') {
                $deal_savings = floatval($obj["msrp"]) - floatval($obj["sales_price"]);
                $html .= '<div class="mb-2">
                        <span class="d-block" style="color: #343a40;font-size: .8rem; font-weight: 700;">BLACK FRIDAY DEAL</span>
                        <h3 class="my-0">$' . number_format($obj["sales_price"], 2) . '</h3>
                        <span class="d-inline-block" style="padding: 0 3px; background: red;font-size: .8rem; color: #fff; font-weight: 700;">Save $' . number_format($deal_savings, 2) . '</span><span class="d-inline-block" style=" margin-left: 5px;color: #343a40; font-size: .8rem; font-weight: 500;"> Was $' . number_format($obj["msrp"], 2) . '</span>
                        </div>';
            } else {
                $html .= '<h3 class="mb-1">$' . number_format($obj["msrp"], 2) . '</h3>';
            }
            $html .= '<p class="text-muted">SKU: ' . $obj["sku"] . '</p>';
            if ($obj["pickup_only"] == 'True') {
                $html .= '<p style="color: #005797"><i class="fa fa-map-marker" aria-hidden="true"></i> In-Store Pickup Only</p>';
            }

            if ($obj["sales_price"] != 'Null') {
                $final_price = $obj["sales_price"];
            } else {
                $final_price = $obj["msrp"];
            }
...
<a class="a2a_button_email"></a><script async src="https://static.addtoany.com/menu/page.js"></script>
    ...
    <div class="col-md-12 striped_details" style="margin:0;"><h2 style="color:white; background-color: #005797; padding: 5px; margin-bottom: 0 !important;">Product Details</h2>' . $obj["description"] . '</div>';
            $test = $data->query("SELECT * FROM custom_equipment WHERE cat_one = '" . $obj["cat_one"] . "' AND parent_cat != 'Attachment'") or die($data->error);
            if ($test->num_rows > 1) {
                if ($test->num_rows < 5) {
                    $html .= '<style>@media screen and (min-width: 992px) {.slick-dots {display: none;}}</style>';
                }
                $html .= '<div class="col-md-12 text-center"><h2 style="padding: 30px 5px;">Other Products From Category</h2></div>
                <style> .other_product_slide .slick-prev:before, .other_product_slide .slick-next:before {font-size: 24pt;color: #005797;}.other_product_slide .slick-prev{left: -5vw;}.other_product_slide .slick-next{right: -5vw;}.other_product_slide .slick-dots li button, .slick-dots li.slick-active button:before {color: #005797;}.other_product_slide .slick-dots li button:before {font-size: 50px;}</style>
                <div class="col-12 col-lg-10 col-xl-12 mx-auto"><div class="row other_product_slide">';

                while ($tomek = $test->fetch_array()) {
                    if (!is_null($tomek) && $tomek['title'] != $obj["title"]) {
                        $trim_text = '';
                        if (strlen($tomek["title"]) >= 48) {
                            $trim_text = mb_substr(str_replace('-', ' ', $tomek["title"]), 0, 48) . '...';
                        } else {
                            $trim_text = str_replace('-', ' ', $tomek["title"]);
                        }
                        $html .= '
                        <div class="card col text-center" style="margin-bottom: 20px; border-radius: 0px; border: transparent;">
                            <div class="product-image6">
                                <a href="' . $_SERVER['REQUEST_URI'] . '/' . $tomek["title"] . '">
                                    <img class="pic-1 img-responsive fadeIn" style="object-fit: cover; width: 100%; height: 240px;" src="../../../img/custom/' . str_replace(['[', '"', ']'], '', $tomek["eq_image"]) . '">
                                </a>
                            </div>
                            <div class="product-content" style="padding: 20px;">
                                <h3 class="title text-center"><a href="' . rtrim($_SERVER['REQUEST_URI'], $obj["title"]) . $tomek["title"] . '" style="font-family:Bebas Neue, cursive; font-size:1.4rem;">' . $trim_text . '</a></h3>';
                        if ($tomek["sales_price"] != 'Null') {
                            $deal_savings = floatval($tomek["msrp"]) - floatval($tomek["sales_price"]);
                            $html .= '<div class="mb-2">
                                            <span class="d-block" style="color: #343a40;font-size: .8rem; font-weight: 700;">BLACK FRIDAY DEAL</span>
                                            <h3 class="my-0">$' . number_format($tomek["sales_price"], 2) . '</h3>
                                            <span class="d-inline-block" style="padding: 0 3px; background: red;font-size: .8rem; color: #fff; font-weight: 700;">Save $' . number_format($deal_savings, 2) . '</span><span class="d-inline-block" style=" margin-left: 5px;color: #343a40; font-size: .8rem; font-weight: 500;"> Was $' . number_format($tomek["msrp"], 2) . '</span>
                                            </div>';
                        } else {
                            $html .= '<h3 class="mb-1 text-center" style="font-size:1.4rem;">$' . number_format($tomek["msrp"], 2) . '</h3>';
                        }
            ...
            <div class="card text-center" style="margin-bottom: 20px; border-radius: 0px; border: transparent;">
                <div class="product-image6">
                    <a href="' . $_SERVER['REQUEST_URI'] . '/' . $d["page_name"] . '">
                        <img class="pic-1 img-responsive fadeIn" style="object-fit: cover; width: 100%; height: 240px;" src="' . $d["cat_img"] . '">
                    </a>
                </div>
                <div class="product-content" style="padding: 20px;">
                ...
                <div class="social">
                    ...
                    <small style="padding: 3px; display: block; background: #fff; color: #333; height: auto">' . $d["page_desc"] . '<br><br></small>
                            ...
                            $trim_text = '';
                            if (strlen($f["title"]) >= 48) {
                                $trim_text = mb_substr(str_replace('-', ' ', $f["title"]), 0, 48) . '...';
                            } else {
                                $trim_text = str_replace('-', ' ', $f["title"]);
                            }
                            $catOut .= '<div class="col-md-6 col-lg-4" data-boxval="' . $f["id"] . '" >
    <div class="card text-center" style="margin-bottom: 20px; border-radius: 0px; border: transparent;">
        <div class="product-image6">
            <a href="' . $_SERVER['REQUEST_URI'] . '/' . $f["title"] . '">
                <img class="pic-1 img-responsive fadeIn" style="object-fit: cover; width: 80%; height: 240px;" src="../img/custom/' . $mainImage . '">
        ...
        <div class="product-content" style="border: transparent;">
            <h3 class="title text-center"><a href="' . $_SERVER['REQUEST_URI'] . '/' . $f["title"] . '" style="font-family:Bebas Neue, cursive">' . $trim_text . '</a></h3>';
                            if ($f["sales_price"] != 'Null') {
                                $deal_save = floatval($f["msrp"]) - floatval($f["sales_price"]);
                                $catOut .= '<div class="mb-2">
                <span class="d-block" style="color: #343a40;font-size: .8rem; font-weight: 700;">BLACK FRIDAY DEAL</span>
                <h3 class="my-0">$' . number_format($f["sales_price"], 2) . '</h3>
                <span class="d-inline-block" style="padding: 0 3px; background: red;font-size: .8rem; color: #fff; font-weight: 700;">Save $' . number_format($deal_save, 2) . '</span><span class="d-inline-block" style=" margin-left: 5px;color: #343a40; font-size: .8rem; font-weight: 500;"> Was $' . number_format($f["msrp"], 2) . '</span>
                </div>';
                            } else {
                                $catOut .= '<h3 class="mb-1 text-center" style="font-size:1.4rem;">$' . number_format($f["msrp"], 2) . '</h3>';
                            }
