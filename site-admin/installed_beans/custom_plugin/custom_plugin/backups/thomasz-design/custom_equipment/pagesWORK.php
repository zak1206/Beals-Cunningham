...
$uniqIDs = md5(date('djsi'));
            ...
            if($theprice != '' && $obj["msrp"] != '') {
                ...
                $ifcart = '<button class="btn btn-dark save-later" data-eqid="'.$obj["id"].'" data-eqname="'.$obj["title"].'" data-eqtype="custom" data-price="'.$obj["msrp"].'" data-url="'.$actual_link.'" data-tabs="custom_equipment" data-itemid="'.$uniqIDs.'"> Add To Cart <i class="fa fa-shopping-cart"></i></button>';
            } elseif($theprice != '') {
            ...
            $locasel .= '<select class="form-control" name="ecomlocation" id="ecomlocation">';
            ...
            $html .= '<div class="clearfix"></div>'.$pricepresent.'<br><button class="btn btn-success parts-button moreinfo" style="background-color:#383838; border-color: #383838;" data-url="'.$actual_link.'" data-equipment="Custom - '.$title.'" data-toggle="modal" data-target="#exampleModal">Request A Quote</button> ' . $saveButton . '<br><br><div style="max-width: 50%
">'.$locasel.'</div><br>'.$ifcart.'<br><br>
<!-- AddToAny BEGIN -->
...
<!-- AddToAny END --><br><br><div style="font-size:30px">' . $starsNow . ' <br>' . $readRevs . ' <a style="font-size: 12px" href="javascript:void(0)" class="reviewit" data-neweqids="' . $obj["id"] . '"></a></div><br><br>' . $optLinksOut . '<br><br><div class="offer-titles-set"></div></div>';
            ...
            $html .= '<h1 class="big-fat-title" style="background:#383838; padding: 10px; color:#fff; display:block;">Product Details</h1>';
