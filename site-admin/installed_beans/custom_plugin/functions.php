...
class customClass
{
    ...
    function updateProduct($post)
    {
        ...
        $img = $_POST["cat_img"];
        $data->query("UPDATE custom_equipment SET eq_image = '\[\"$img\"\]' , sales_price = '" . $post["sales_price"] . "', msrp = '" . $data->real_escape_string($post["msrp"]) . "', parent_cat = '" . $post["parent_cat"] . "', cat_one = '" . $post["offers_active"] . "', description = '" . $post["description"] . "',features = '" . $post["features"] . "', title = '" . $post["title"] . "' WHERE id = '" . $post["proid"] . "'") or die($data->error);
    }
