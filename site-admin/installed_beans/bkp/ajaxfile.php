...
</div> <button type="button" class="btn btn-danger btn-sm no-print" onclick="deleteOrder()"><i class="fa fa-trash mr-2"></i> Delete Order</button></div>
                </div>
                <div class="card-body">
                    <div class="row">
...
if ($act == 'savecat') {
    ...
    if (isset($cat_id) && $cat_id != null) {
        ...
        $results = array("status" => "good", "message" => '<div class="alert alert-success"><strong>Awesome!.</strong><br><p>Your category has been successfully updated. </p></div>', "page_id" => $cat_id);
    } else {
        ...
        if ($a->num_rows == 0) {
            ...
            if ($b->num_rows == 1) {
                $results = array("status" => "good", "message" => '<div class="alert alert-success"><strong>You new category has been saved.</strong><br><p>You can continue to edit or you can go back to the category page.</p></div>', "page_id" => $b["id"]);
            }
        } else {
            //already exist//
            $results = array("status" => "bad", "message" => '<div class="alert alert-warning"><strong>Whoops!.</strong><br><p>It appears you have a category with the same name already active.<br>Please re-name your category or try accessing it from the category menu.</p></div>', "page_id" => '');
        }
    }
...
if ($act == 'getproducts') {
    ...
    while ($b = $a->fetch_array()) {
        ...
        if ($b["parent_cat"] != null) {
            $c = $data->query("SELECT sub_title FROM custom_equipment_pages WHERE id = '" . $b["parent_cat"] . "'");
            $d = $c->fetch_array();
            $primaryCat = $d["sub_title"];
        } else {
            $primaryCat = '-Not Set-';
        }
        ...
        $html .= '<tr>
                ...
                <td style="text-align: right"><button class="btn btn-secondary" onclick="editProduct(1)"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button> <button class="btn btn-danger" onclick="StartDeleteProd(' . $b['id'] . ')"><i class="fa fa-trash" aria-hidden="true"></i></button></td>
            </tr>';
    }
...
if ($act == 'createproductfin') {
    ...
    if (isset($prodId)) {
        ...
        $results = array("status" => "good", "message" => '<div class="alert alert-success"><strong>Awesome!.</strong><br><p>Your Product has been successfully CREATED. <br>You can now go back to the list or continue to edit this Product.</p></div>', "prod_id" => $prodId);
    }
...
if ($act == 'deleteattrs') {
    $id = $_REQUEST["id"];
    $data->query("DELETE FROM custom_equipment_product_attr_childs WHERE id = '$id'");
}
...
if ($act == 'edit_coupon') {
    ...
    //ROW START//
    $html .= '<br><div class="row">';
    $html .= '<div class="col-md-4"><label style="font-weight: bold; margin: 0">Expire Date</label><br><small>If coupon does not expire, leave blank</small><br><input class="form-control" type="date" name="exp_date" id="exp_date" value="' . date($date_expire, 'm/d/Y') . '" placeholder="example. 25% or 10.00"></div>';
