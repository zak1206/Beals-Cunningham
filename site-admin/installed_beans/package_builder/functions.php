...
class packageclass
{
    ...
    function getEquipmentLink($equipid)
    {
        if (file_exists('../../inc/harness.php')) {
            include('../../inc/harness.php');
        } else {
        ...
        $a = $data->query("SELECT equip_link, eq_image FROM deere_equipment WHERE title = '$equipid'") or die($data->error);
        $b = $a->fetch_array();
    ...
    function getPackageImps($lines)
    {
        ...
        for ($i = 0; $i < count($eachline); $i++) {
            ...
            if ($b["type"] != 'implement') {
                ...
                $html .= '<div class="dropitemsin" data-id="' . $b["name"] . '" style="background: #F8CE54; border: solid .6px #eee; padding: 6px 10px; color: black; margin-bottom: 10px; position: relative; vertical-align:bottom;"><p style="width: 80%;">' . $b["name"] . '<span style="font-size: .6rem;"></span>  Category:(' . $b["category"] . ') - List Price: ' . $b["price"] . '</p><a style="width: 20%;" class="removeites"><i class="fa fa-trash" aria-hidden="true"></i> Remove</a></div>';
            }
        }
        return $html;
    }
    ...
    function getPackageAtts($lines)
    {
        ...
        for ($i = 0; $i < count($eachline); $i++) {
            ...
            if ($b["type"] != 'attachment') {
                ...
                $html .= '<div class="dropitemsin" data-id="' . $b["name"] . '"style="background: #F8CE54; border: solid .6px #eee; padding: 6px 10px; color: black; margin-bottom: 10px; position: relative; vertical-align:bottom;"><p>' . $b["name"] . '<span style="font-size: .6rem;"></span> - List Price: ' . $b["price"] . '<span style="float: right;"></span></p><a class="removeites"><i class="fa fa-trash" aria-hidden="true"></i> Remove</a></div>';
            }
        }
        return $html;
    }
    ...
    function finishAddLine($post)
    {
        ...
        $a = $data->query("INSERT INTO attachment_data SET
          name =  '" . $data->real_escape_string($newtitle) . "',
          ...
          add_image = '" . $data->real_escape_string($image) . "',
          type =  '" . $data->real_escape_string($addtype) . "',
          updated = '" . time() . "',
          active = 'true'") or die($data->error);
    ...
    function finishEditLine($post)
    {
        ...
        $pack = explode(',' ,trim($post["packs"], '*'));

        $packdata = str_replace(' ', '',json_encode($pack));
...
//        var_dump($post);
        ...
        $b = $data->query("UPDATE attachment_data SET name =  '" . $data->real_escape_string($newtitle) . "', description =  '" . $data->real_escape_string($desc) . "', price =  '" . $data->real_escape_string($price) . "', add_image = '" . $data->real_escape_string($image) . "',equipment_names = '" . $data->real_escape_string($packdata) . "'  WHERE id = '$id'");

        if($post["category"] != '') {
            $a = $data->query("UPDATE attachment_data SET description = '" . $data->real_escape_string($desc) . "', add_image = '" . $data->real_escape_string($image) . "', equipment_names = '" . $data->real_escape_string($packdata) . "' WHERE category = '" . $data->real_escape_string($category) . "' AND type = 'implement'");
        } else {
    ...
    function getPackageTitles()
    {
        ...
        $a = $data->query("SELECT equip_id FROM package_equipment WHERE active = 'true' ORDER BY equip_id ASC");
        while ($b = $a->fetch_array()) {
            $packs[] = $b["equip_id"];
        }
        return $packs;
    ...
    function updateOfferDetails($post)
    {
        if (file_exists('../../inc/harness.php')) {
            include('../../inc/harness.php');
        } else {
        ...
        $id = $post["pack_id"];
        if($post["add_discount"] == 0) {
            $addamount = '';
        } else {
            $addamount = $post["add_discount"];
        }
        $a = $data->query("UPDATE package_equipment SET starting_rate = '" . $data->real_escape_string($post["start_int"]) . "', additional_discounts = '" . $data->real_escape_string($addamount) . "' WHERE id = '$id'");
    }
