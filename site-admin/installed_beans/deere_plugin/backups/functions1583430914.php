        ...
        if($theArs["status"] == 'Good'){
            ...
            for($i=0; $i < count($deereEquip); $i++){
                ...
                if($g->num_rows > 0){
                    ...
                    $data->query("INSERT INTO deere_pages SET page_name = '".$data->real_escape_string($deereEquip[$i]["page_name"])."', page_title = '".$data->real_escape_string($deereEquip[$i]["page_title"])."', page_content = '".$data->real_escape_string(htmlspecialchars_decode($deereEquip[$i]["page_content"]))."', active = '".$data->real_escape_string($deereEquip[$i]["active"])."', created = '".$data->real_escape_string($deereEquip[$i]["created"])."', last_edit = '".$data->real_escape_string($deereEquip[$i]["last_edit"])."', page_lock = '".$data->real_escape_string($deereEquip[$i]["page_lock"])."', page_type = '".$data->real_escape_string($deereEquip[$i]["page_type"])."', page_desc = '".$data->real_escape_string($deereEquip[$i]["page_desc"])."', page_js = '".$data->real_escape_string($deereEquip[$i]["page_js"])."', equipment_content = '".$data->real_escape_string(htmlspecialchars_decode($deereEquip[$i]["equipment_content"]))."', cat_type = '".$data->real_escape_string($deereEquip[$i]["cat_type"])."', cat_img = '".$data->real_escape_string($deereEquip[$i]["cat_img"])."'")or die($data->error);
                }
            }
