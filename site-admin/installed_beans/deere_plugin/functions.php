...
class deereClass
{
    ...
    function updateProduct($post){
        ...
        $data->query("UPDATE deere_equipment SET eq_image = '".$data->real_escape_string($post["cat_img"])."', price = '".$post["pricepoint"]."', dealer_price = '".$data->real_escape_string($post["msrp"])."', quick_links_active = '".$post["quick_links_active"]."', offers_active = '".$post["offers_active"]."', videos_active = '".$post["videos_active"]."', access_active = '".$post["access_active"]."', extra_content = '".$data->real_escape_string($post["promoarea"])."' WHERE id = '".$post["proid"]."'");
        ...
        if($theArs["status"] == 'Good'){
            ...
            for($i=0; $i < count($deereEquip); $i++){
                ...
                if($g->num_rows > 0){
                    ...
                    $data->query("INSERT INTO deere_pages SET page_name = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["page_name"]))."', page_title = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["page_title"]))."', page_content = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["page_content"]))."', active = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["active"]))."', created = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["created"]))."',  page_type = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["page_type"]))."', page_desc = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["page_desc"]))."', page_js = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["page_js"]))."', equipment_content = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["equipment_content"]))."', cat_type = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["cat_type"]))."', cat_img = '".$data->real_escape_string(html_entity_decode($deereEquip[$i]["cat_img"]))."'")or die($data->error);
                }
            }
...
// use key 'http' even if you send the request to https://...
    ...
    function getUpdatePackage($updatepackage,$systemtime){
        $url  = $updatepackage;
