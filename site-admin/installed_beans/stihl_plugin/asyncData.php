...
if($act == 'openproduct'){
    ...
    $product = $stihl->getProduct($_REQUEST["prod"]);
    //var_dump($product);
...
if($act == 'addneweq'){
    ...
    $parentCats = $stihl->getcats('parent_cat');
    ...
    $cat_three = $stihl->getcats('cat_three');
    ...
    $cat_four = $stihl->getcats('cat_four');
    if(!empty($cat_four) ){
...
if($act == 'processEquip'){
    $stihl->processSingleEquipment($_POST);
}
...
if($act == 'getprodcats'){
    ...
    if($_REQUEST["cattype"] != null && $_REQUEST["catname"] != null){
        ...
        $cats = $stihl->getEquipmentProducts($_REQUEST["cattype"],$_REQUEST["catname"]);

        $categoryOut .= '<h4 style="text-align: center">'.$_REQUEST["catname"].'</h4>';


        $backLins = $stihl->getBackLink($_REQUEST["cattype"],$_REQUEST["catname"]);
...
if($act == 'filtercustomcats'){
    $realCats = $stihl->getEqCats($_REQUEST["filter"]);
...
if($act == 'pullupdates'){
    echo $stihl->getUpdatePackage($_REQUEST["pack"]);

}

if($act == 'createbean'){
    $stihl->createEqBean($_REQUEST["bean_id"]);
}
