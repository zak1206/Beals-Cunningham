...
class deere_equipment
{
    ...
    function in_array_r_deeres($item, $array)
    {
    ...
    function page($page)
    {
        ...
        if (isset($_SESSION["target_ads"])) {
            ...
            //check for dups//
            if ($this->in_array_r_deeres($equipBase, $session)) {
                ///DONT ADD A THING///
            } else {
            ...
            for ($i = 0; $i < count($eqipInfor); $i++) {
                if ($i < 10) {
                    ...
                    if ($a->num_rows > 0) {
                        ...
                        $eqipInfo[] = array(
                            "equiplink" => $eqipInfor[$i]["equiplink"], "equipbase" => $eqipInfor[$i]["equipbase"], "capture_date" => $eqipInfor[$i]["capture_date"], "image" => $image, "parent_cat" => $eqipInfor[$i]["parent_cat"], "cat_one" => $eqipinf[$i]["cat_one"],
...
<section id="review-top">
        ...
        <section id="reviewdata">';
            ...
            if ($numofdays > 10 && $apipricestatus == 'true') {
                ...
                // echo 'time to update';
                include('inc/config.php');
                $data->query("UPDATE deere_equipment SET price = '$apiprice', last_updated = '$time' WHERE title ='$equipname' AND active = 'true'") or die($data->error);
            ...
            if ($b["dealer_price"] != '') {
                $pricepresent = '<span class="startingat" style="font-weight: bold; font-size: 20px;">Starting At:</span><span class="equipprice">' . $b["dealer_price"] . '</span>';
                ...
                $pricepresent = '<span class="startingat" style="font-weight: bold; font-size: 20px;">Starting At:</span><span class="equipprice">' . $b["price"] . '</span>';
                //$ifcart = '<button style="display: none" class="btn btn-dark save-later" data-eqid="'.$b["id"].'" data-eqname="'.$b["title"].'" data-eqtype="deere" data-price="'.$b["price"].'" data-url="'.$actual_link.'"> Add To Cart <i class="fa fa-shopping-cart"></i></button>';
            } else {
                $pricepresent = '';
                //$ifcart = '';
            }
            ...
            if($b['finance_url'] != '') {
                $financebtn = '<a href="'.$b['finance_url'].'" target="_blank" class="new-main-btn btn" rel="noopener" style="background: #FFDE00;">Apply For Financing</a>';
            ...
            if ($offerZZ != null) {
                ...
                $offerZZa .= str_replace('/assets/','https://www.deere.com/assets/',$offerZZ);
                $offerZZa .= '</div>';
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title testing">Request More Info</h5>
      ...
      <div class="modal-footer">
            ...
            <div class="col-md-4 col-sm-12">
                <h1 class="model"><span class="brand"></span>' . strtoupper(str_replace('-', ' ', $title)) . '<br></h1>
                ...
                ' . $pricepresent . '<br><button style="margin-right: 20px;"class="btn btn-success moreinfo" data-url="' . $actual_link . '" data-equipment="John Deere - ' . $title . '" data-toggle="modal" data-target="#exampleModal">' . $ctatext . '</button>' .$ifcart . $financebtn.'<br>'.$bobtn.$extra.'<br><a href="'.$brochlink . '" target="_blank" class="brochure-link" style="font-weight: bold; color: green;">View Product Brochure</a><br><div class="a2a_kit a2a_kit_size_32 a2a_default_style" style="line-height: 32px;">
...
<div style="clear: both;"></div></div><script async="" src="https://static.addtoany.com/menu/page.js"></script><br>
    ...
    <div id="used-car-target">' . $usedCar . '</div>
            ...
            $output .= '<section id="new-equip-tabs" style="position: sticky; z-index: 5; top: 0;">
...
';
            ...
            $content[] = array("page_name" => $page, "page_title" => 'TEST', "page_content" => $output, "active" => true, "created" => time(), "last_edit" => '', "last_user" => '', "page_lock" => 'none', "page_type" => '', "page_desc" => '', "check_out" => false, "check_out_date" => '', "page_js" => '', "dependants" => $arsOut);
            return $content;
        } else {
            ...
            <div class="card" style="margin-bottom: 20px; border-radius: 0px; border: transparent;">
                <div class="product-image6">
                    <a href="' . $_SERVER['REQUEST_URI'] . '/' . $d["page_name"] . '">
                        <img class="pic-1 img-responsive fadeIn" style="padding: 20px; height: 250px; object-fit: cover;" src="' . $d["cat_img"] . '">
                    </a>
                </div>           
                <div class="product-content" style="padding: 20px">
                    ...
                    <small style="padding: 3px; min-height: 150px; display: block">' . $d["page_desc"] . '</small>
                            ...
                            if ($equip_content[$i]["type"] == 'htmlarea') {
                                ...
                                if ($f["price"] != null) {
                                    $priceOuts = '<span class="product-new-label" style="color: #333; font-weight:bold; font-size: 14px; padding-left: 20px; display: none;">MSRP STARTING AT $' . $f["price"] . '</span>';
                                }
            ...
            <div class="card" style="border: transparent;">
                <div class="product-image">
                    <a href="' . $_SERVER['REQUEST_URI'] . '/' . $f["title"] . '">
                        <img class="pic-1 img-responsive fadeIn" style="padding: 20px; height: 250px; object-fit: cover;" src="../img/equip_images/' . $equipImg . '">
                ...
                <div class="product-content" style="border: transparent;">
                    <h3 class="title" style="margin-bottom: 0; background: #346635; color: white; font-size: 20px;"><a style="color: white; padding-left: 20px;"href="' . $_SERVER['REQUEST_URI'] . '/' . $f["title"] . '">' . strtoupper(str_replace('-', ' ', $f["title"])) . '</a></h3>
