<?php
class honda_equipment
{
    function page($page)
    {
        include('inc/config.php');
        $a = $data->query("SELECT * FROM honda_equipment WHERE title = '$page'") or die($data->error);
        if ($a->num_rows > 0) {
            ...
            $html .= '<!--modal begin-->
    <!-- Modal -->
    <div class="modal fade" id="myModalEq" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {form}Honda_Equipment_Quote{/form}
                </div>
...
<label>Your Review</label><br>
                        ...
                        <input type="hidden" name="eqiptype" id="eqiptype" value="honda">
            ...
            $html .= '<div class="col-md-8" style="padding:0"><img style="width:50%; display:block; margin:0px auto;" src="img/Honda/' . $image . '"></div>';
            ...
            if ($price != null) {
                if (isset($saveFlag)) {
                    ...
                    $saveButton = '<!--<button class="btn btn-default save-later" data-eqid="' . $obj["id"] . '" data-eqname="' . $page . '" data-eqtype="honda" data-price="' . str_replace(",", "", $price) . '" data-url="' . $url . '"><i class="fa fa-shopping-cart"></i> Add To Cart</button>-->';
                }
            } else {
                $saveButton = '';
            }
            ...
            $html .= '<div class="col-md-4"><h1 class="eq_title big-honda-title">' . $tt . '<br><span class="sub-h1">' . $outTitleSub . '</span></h1>' . $bullethtml . '<br><br>' . $msrp . '' . $priceOut . '<br><br> <button class="btn btn-danger parts-button neweqreq moreinfo" data-equipment="Honda - ' . $title . '" data-url="' . $actual_link . '" data-toggle="modal" data-target="#myModalEq">Request a Quote</button> ' . $saveButton . '<br><br>
<!-- AddToAny BEGIN -->
...
<!-- AddToAny END --><br><br><div style="font-size:30px">' . $starsNow . ' <br>' . $readRevs . ' <a style="font-size: 12px" href="javascript:void(0)" class="reviewit" data-neweqids="' . $obj["id"] . '"></a></div><br><br>' . $optLinksOut . '<br><br><div class="offer-titles-set"></div></div>';
            ...
            if ($obj["videos_active"] == 'true') {
                ...
                if (!empty($videos)) {
                    ...
                    if (count($videos) > 1) {
                        ...
                        $html .= '<div class="vidoverall" style="padding:20px; display:none;"><button type="btn" class="close close-vids" style="padding:10px"><i class="fa fa-times"></i> Close</button><div class="clearfix"></div><div class="embed-responsive embed-responsive-16by9"><iframe id="hondavids" class="embed-responsive-item" src="https://www.youtube.com/embed/' . $firstVid . '?rel=0" allowfullscreen></iframe></div></div>';
                        $html .= '</div>';
                        ...
                        if (!empty($vidOut)) {
                            ...
                            ///echo "This is ". $vids;
                            $data->query("UPDATE honda_equipment SET video_storage = '" . $data->real_escape_string($vids) . "' WHERE id = '" . $obj["id"] . "'") or die($data->error) or die($data->error);
                        } else {
                        ...
                        $html .= '<h2 style="color: #fff; padding: 25px">' . $videosTitle . '</h2>';
                        $html .= '<div class="col-md-12" style="padding: 0">';
                        $html .= '<div class="embed-responsive embed-responsive-16by9"><iframe id="hondavids" class="embed-responsive-item" src="https://www.youtube.com/embed/' . $firstVid . '?rel=0" allowfullscreen></iframe></div>';
                        $html .= '</div>';
                        $html .= '<div class="col-md-12" style="padding: 0; text-align: center">';
            ...
            $html .= '<h1 class="big-honda-title" style="background: #CC0000; padding: 10px; color: #fff; display: block">Specs</h1>';
            ...
            if ($obj["offers_active"] == 'true') {
                ...
                if ($lastTime > 10) {
                    ...
                    foreach ($finnalyOfffers as $offerlink) {
                        ...
                        $offerLinkClean = str_replace('/html/honda/us/en/website/', 'en/', $offerlink);
                        $offersLinkNow = 'https://www.honda.com/' . $offerLinkClean . '/index.json';
                        ...
                        if (is_array($objOffers) && $objOffers["Page"]["special-offers"]["ESIFragments"] != null) {
                            if (strpos($objOffers["Page"]["special-offers"]["ESIFragments"], 'https://www.honda.com') !== false) {
                                $jsonOfferOut = $this->file_get_contents_curl($objOffers["Page"]["special-offers"]["ESIFragments"]);
                            } else {
                                $jsonOfferOut = $this->file_get_contents_curl('https://www.honda.com' . $objOffers["Page"]["special-offers"]["ESIFragments"]);
                            }

                            //var_dump($objOffers["Page"]["special-offers"]["ESIFragments"]);
                            $content = str_replace('srcset="', 'srcset="https://honda.com', $jsonOfferOut);
                    ...
                    if ($offerZZ != null) {
                        ...
                        $data->query("UPDATE honda_equipment SET offers_storage = '" . $data->real_escape_string($offerZZa) . "', last_updated = '" . time() . "' WHERE id = '" . $obj["id"] . "'") or die($data->error);
            ...
            if ($obj["access_active"] == 'true') {
                ...
                if ($lastTime > 30) {
                    if ($obj["accessories"] != 'https://www.honda.com') {
                        ...
                        $data->query("UPDATE honda_equipment SET accessories_storage = '" . $data->real_escape_string($assOut) . "', last_updated = '" . time() . "' WHERE id = '" . $obj["id"] . "'") or die($data->error);
            ...
            $a = $data->query("SELECT * FROM honda_pages WHERE page_name = '$page' AND active = 'true'");
            ...
            if ($a->num_rows > 0) {
                ...
                $categoryOut = preg_replace_callback('#{prodcat}data{/prodcat}#', function ($match) use ($page) {
                    ...
                    include('inc/config.php');
                    $a = $data->query("SELECT * FROM honda_pages WHERE page_name = '$page'");
                    $b = $a->fetch_array();
                    ...
                    for ($i = 0; $i < count($equip_content); $i++) {
                        if ($equip_content[$i]["type"] == 'category') {
                            $c = $data->query("SELECT * FROM honda_pages WHERE page_name = '" . $equip_content[$i]["title"] . "'");
                            $d = $c->fetch_array();
                ...
                <div class="product-content" style="padding: 20px">
                    ...
                    <small style="padding: 3px; min-height: 150px; display: block">' . $d["page_desc"] . '</small>
                            ...
                            if ($equip_content[$i]["type"] == 'htmlarea') {
                                ...
                                $e = $data->query("SELECT title, eq_image, id, price FROM honda_equipment WHERE title = '" . $equip_content[$i]["title"] . "'") or die($data->error);
                                $f = $e->fetch_array();
            ...
            <div class="card" style="border: transparent;">
                <div class="product-image">
                    <a href="' . $_SERVER['REQUEST_URI'] . '/' . $f["title"] . '">
                        <img class="pic-1 img-responsive fadeIn" src="../img/Honda/' . $equipImg . '">
                ...
                <div class="product-content" style="border: transparent;">
                    <h3 class="title" style="margin-bottom: 0; background: #CC0000; color: white; font-size: 20px;"><a style="color: white; padding-left: 20px;"href="' . $_SERVER['REQUEST_URI'] . '/' . $f["title"] . '">' . str_replace('-', ' ', $f["title"]) . '</a></h3>
                ...
                if (strpos($_SERVER['REQUEST_URI'], 'Used-Equipment') != true) {
                    ...
                    $css[] = 'inc/mods/honda_equipment/honda-output/css/lightslider.css';
                    $css[] = 'inc/mods/honda_equipment/honda-output/css/main.css';
                    $css[] = 'inc/mods/honda_equipment/honda-output/css/jquery.paginate.css';

                    $js[] = 'inc/mods/honda_equipment/honda-output/js/lightslider.js';
                    $js[] = 'inc/mods/honda_equipment/honda-output/js/jquery.paginate.js';
                    $js[] = 'inc/mods/honda_equipment/honda-output/js/new-output.js';
                    $js[] = 'inc/mods/honda_equipment/honda_functions.js';
