...
class machine_finder_two{
    function page($page){
        ...
        if(is_numeric($id) && strpos($_SERVER['REQUEST_URI'], 'Used-Equipment') !== false){

            $a = $data->query("SELECT * FROM used_equipment WHERE id = $id");
            $b = $a->fetch_array();


            $eqName = $b["manufacturer"] . ' ' . $b["model"];
            ...
            if($a->num_rows >0) {
                ...
                $htmlOut .= '<div class="col-md-7 testing">';
                ...
                $theImages = $images["image"];
                    ...
                    if(count($theImages) > 0) {
                        ...
                        for ($i = 0; $i < count($theImages); $i++) {
                            $imgBox .= '<div><img style="width: 100%" src="' . $theImages[$i]["filePointer"] . '"></div>';
                            $imgThumb .= '<div style="border:solid 3px #fff; max-height: 200px; overflow: hidden"><img style="width: 100%" src="' . $theImages[$i]["filePointer"] . '"></div>';
                        }
                    }else{
                        $imgBox .= '<div><img style="width: 100%" src="https://via.placeholder.com/150?text=No Image"></div>';
                        $imgThumb .= '';
                    }
...
' . $imgThumb . '
                ...
                if($b["videos"] != null){
                    $htmlOut .= '<iframe src="https://player.vimeo.com/video/'.$b["videos"].'" width="640" height="360" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>';
                }

                $htmlOut .= '<h3>Description</h3><p>' . $b["description"] . '</p>';
                $htmlOut .= '</div>';
                $htmlOut .= '<div class="col-md-5">';
                $htmlOut .= '<h2>' . $b["manufacturer"] . ' ' . $b["model"] . '</h2>';
                if ($b["price"] != null) {
                ...
                if ($b["operationHours"] != '' && $b["operationHours"] != '0') {
                    $htmlOut .= '<tr><td><strong>Hours:</strong></td><td>' . $b["operationHours"] . '</td></tr>';
                }

                    if ($b["acres"] != '' && $b["acres"] != '0') {
                        $htmlOut .= '<tr><td><strong>Acres (Estimated):</strong></td><td>' . $b["acres"] . '</td></tr>';
                    }

                if($b["separatorHours"] != '' && $b["separatorHours"] != '0'){
                    $htmlOut .= '<tr><td><strong>Separator Hours:</strong></td><td>' . $b["separatorHours"] . '</td></tr>';
                }

                if($b["bale_count"] != '' && $b["bale_count"] != '0'){
                    $htmlOut .= '<tr><td><strong>Bale Count:</strong></td><td>' . $b["bale_count"] . '</td></tr>';
                }

                $htmlOut .= '<tr><td><strong>Stock / DSID #:</strong></td><td>' . $b["stockNumber"] . '</td></tr>';
                $htmlOut .= '<tr><td><strong>Location:</strong></td><td>' . $b["city"] . '</td></tr>';
                $htmlOut .= '</table>';
...
<!-- AddToAny END -->';
                ...
                $content[] = array("page_name" => $page, "page_title" => $eqName, "page_content" => '<div class="container">' . $htmlOut . '</div>', "active" => true, "created" => time(), "last_edit" => '', "last_user" => '', "page_lock" => 'none', "page_type" => '', "page_desc" => '', "check_out" => false, "check_out_date" => '', "page_js" => '');

                return $content;
            }else{
                    $content[] = array("page_name" => 'Equipment Not Found', "page_title" => 'Equipment Not Found', "page_content" => '<div class="container"><div class="jumbotron"><h2>Equipment Not Found</h2><p>The equipment you are looking for has either been sold or removed.</p></div></div>', "active" => true, "created" => time(), "last_edit" => '', "last_user" => '', "page_lock" => 'none', "page_type" => '', "page_desc" => '', "check_out" => false, "check_out_date" => '', "page_js" => '');
                    return $content;
                }
            }
        }else{
                    ...
                    while ($b = $a->fetch_assoc()) {
                        ...
                        if ($_POST["viewtype"] == 'list') {
                           ...
                           <br><br>
                            ...
                            $html .= '<div class="col-sm-2 col-md-6 col-lg-3 col-sm-4">
