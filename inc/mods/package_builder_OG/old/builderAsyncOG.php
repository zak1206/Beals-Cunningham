...
if($act == 'getimplements'){
    ...
    echo $html;
      ...
      //  $details = $builder->getEquipDetail($equip["equip_id"]);
        ...
        $html .= '<div class="row">';
        $html .= '<div class="col-md-6"><img src="img/equip_images/' . $image[0] . '" class="img-responsive"><a class="text-center" href="' . $equip["brochure_link"] . '" target="_blank">View Brochure</a><div class="panel-group">
  ...
  </div></div><div class="col-md-6"><h2 style="font-family: Arial, sans-serif;">Base Price ' . $price . '</span></h2> <div class="clearfix"></div>';
        ...
        foreach ($uniqueCatArr as $cat) {
            ...
            for ($i = 0; $i < count($imps); $i++) {
                if (in_array($cat, $imps[$i])) {
                    $html .= '<option data-name="' . $imps[$i]["title"] . '" data-price="' . $imps[$i]["price"] . '" value="' . $imps[$i]["title"] . '-' . $imps[$i]["price"] . '">' . $imps[$i]["title"] . ' $' . $imps[$i]["price"]. '</option>';
                }
            }
            $html .= '</select>';
        }
...
if($act == 'getattachments'){
  ...
  <div class="input-group-append" style=" margin-right: 10px;">
    <div class="input-group-text">
      <input class="attchecks" name="' . str_replace(' ', '-', $atts[$i]["title"]) . '" type="checkbox" value="' . $atts[$i]["title"] . '-' . $atts[$i]["price"] . '" data-id="' . $atts[$i]["id"] . '">
    </div>
  </div><p>' . $atts[$i]["title"] . '<a style="margin-left: 10px" class="att-info" data-toggle="tooltip" data-placement="top" title="' . $atts[$i]["description"] . '"><i class="fas fa-info-circle"></i></a>  <span style="float: right;position: absolute; right: 10px; font-weight: bold;">$' . $atts[$i]["price"] . '</span></p></div></td></tr>';
        }
...
//                    $data->query("UPDATE deere_equipment SET offers_storage = '".$data->real_escape_string($offerZZa)."' WHERE id = '".$id."'");
    ...
    if ($equip["price"] == $equip["msrp"] || $equip["price"] == '') {
        ...
        $price = '<span class="base-equip-price addit" style="font-family: Arial, sans-serif;"><span class="line-through">$' . $equip["msrp"] . '</span><br>$' . $equip["price"] . '</span>';
    }
    $html .= '<div class="row">';
    $html .= '<div class="col-md-6"><img src="img/equip_images/'.$image[0].'" class="img-responsive"><a class="text-center" href="'.$details["brochure_link"].'" target="_blank">View Brochure</a><div class="panel-group">
    </div>'.$offershtml.'</div><div class="col-md-6"><h2 style="font-family: Arial, sans-serif;">'.$equip["equipment_title"].' - Base Price '.$price.'</span></h2> <div class="clearfix"></div><div class="config-info"><table id="gen-info" class="table">';
    ...
    foreach($_POST as $key => $value)
    {
        if (strstr($key, 'subcategory') || strstr($key, 'equipment'))
        {
            ...
            if ($value == '') {
                ...
                $html .= '<tr><td><b>' . $key . '</b></td><td>' . $newval[0] . '</td><td class="addit">$' . $newval[1] . '</td></tr>';
            }
        }
    }

    $html .= '<tr style="background: transparent"><td style="border: none;"></td><td style="border: none;"><b>Sub Total</b></td><td id="subtal" class="sumit" style="border: none;"></td></tr>';
    $html .= '<tr style="background: transparent"><td style="border: none;"></td><td style="border: none;"><b>Delivery Fee</b></td><td id="delivery" style="border: none;">250.00</td></tr>';
  ...
  <span class="range-slider__value">0</span>
    ...
    $html .= '<tr><td style="width: 32%;"></td><td>Interest Rate</td><td style="width: 37%;"><input id="interestrate" type="number" class="form-control" value="3.9" /></td></tr>';
