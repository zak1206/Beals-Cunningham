...
if ($act == 'getcats') {
    ...
    $html .= '<select name="subcategory" id="subcat-select" class="form-control" onchange="callDoublefunctions(this);" required>';
    ...
    $html .= '</select><script>function callDoublefunctions(selectValue){
...
if ($act == 'getequip') {
    ...
    $html .= '<div class="clearfix"></div><button id="config" type="button" class="btn btn-success" data-cafftrak="Build my package" style="margin-top: 50px;" onclick="startConfig();">Build My Package</button>';
...
if ($act == 'getimplements') {
    ...
    if (count($imps) < 1) {
        ...
        $html .= '<div class="col-lg-6 col-md-12"><img src="img/equip_images/' . $image[0] . '" class="img-responsive"><p style="text-align:center; line-height:1.2"><small style="font-size: 14px">**Subject to inventory availability. Please contact Valley Plains Sales for more information. Picture not representative of base model</small></p><h2 class="equip-title">' . $equip["equipment_title"] . '</h2>' . $details["bullet_points"] . '<a class="text-center" href="' . $brochlink . '" target="_blank">View Brochure</a><div class="panel-group">
...
if ($act == 'getfinal') {
    ...
    $html .= '<div class="row">';
    $html .= '<div class="col-md-6"><img src="img/equip_images/' . $image[0] . '" class="img-responsive"><p style="text-align:center; line-height:1.2"><small style="font-size: 14px">**Subject to inventory availability. Please contact Valley Plains Sales for more information. Picture not representative of base model</small></p><a class="text-center" href="' . $brochlink . '" target="_blank">View Brochure</a><br><button type="button" class="" style="padding: 0;
    border: none;
    background: none;
    color: inherit;" data-toggle="modal" onclick="getOffers(' . $equip["id"] . ')">


  Offers & Discounts</button>
  <div style="clear:both; margin-top: 40px; position:absolute; bottom: 0;"><button type="button" class="btn btn-warning previous" onclick="stepper.previous()">Previous</button></div>
  <div class="panel-group">
    ...
    if ($equip["package"] == "Lawn Tractors and ZTraks") {
        $html .= '<tr><td style="width: 32%;"></td><td class="capture">Months</td><td style="width: 37%;"><div class="range-slider">
        <input class="range-slider__range" id="monthslider" type="range" value="60" min="60" max="60" step="0" list = "monthsetting" onchange="calculateMonthly()" oninput="monthOutputUpdate(value)">
    ...
    } else if ($equip["package"] != "Lawn Tractors and ZTraks") {
        $html .= '<tr><td style="width: 32%;"></td><td class="capture">Months</td><td style="width: 37%;"><div class="range-slider">
        ...
        <tr><td style="width: 32%;"></td><td class="capture">Finance Charge</td><td style="width: 37%; class="capture" id="financeCharge"></td></tr>';
    }
    ...
    //if($equip["package"]=="Lawn Tractors and ZTraks"){$equip["package"]}
    if ($equip["package"] == "Lawn Tractors and ZTraks") {
        $html .= '</table></form><p>All financing subject to credit approval and current offers and discounts available in your area. This is an estimate and may be subject to change. Please finalize quote to determine exact monthly payment.</p><hr /><p style="margin-bottom:55px;"><sup>*</sup>Prices subject to change, see your Valley Plains Sales Rep for details</p>
                <a class="btn btn-success finalize-btn" style="border-radius: 0px;background: #097F0E;padding: 10px 30px;text-transform: uppercase;margin-top: 20px;float: left;position:absolute; bottom: 0;" target="_blank" href="' . $equip["financing_url"] . '">Apply for financing</a><button type="button" class="btn btn-success finalize-btn" data-toggle="modal" data-target="#exampleModal" style="border-radius: 0px;background: #097F0E;padding: 10px 30px;text-transform: uppercase;margin-top: 20px;right:0;position:absolute; bottom: 0;">
                Finalize Quote NOW!!
                </button>';
    } else {
        $html .= '</table></form><p>All financing subject to credit approval and current offers and discounts available in your area. This is an estimate and may be subject to change. Please finalize quote to determine exact monthly payment.</p><hr /><p style="margin-bottom:55px;"><sup>*</sup>Prices subject to change, see your Valley Plains Sales Rep for details</p>
