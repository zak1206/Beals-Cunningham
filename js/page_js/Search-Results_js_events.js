$(function(){
  var monkeyList = new List('search_content', {
  valueNames: ['name'],
  page: 4,
  pagination: true
});
...
$(function(){
  ...
  $(".clickobj").on('click',function(){
        ...
        if($(this).find('.thecheck').length == 1){
            ...
            $(this).append(' <span class="thecheck"> <i class="fa fa-check green-check"></i></span>');
...
$(function(){
 ...
 if($('#searchval').val() != undefined) {
var passsearch = $('#searchval').val();
    $('#searchinput').val(passsearch);
  
  $.ajax({
        url: 'inc/mods/machine_finder/asyncfile.php?action=inputsearch',
        cache: false,
        type: "POST",
        data: {'searchinput': passsearch},
        success: function (result) {
            $('.rezout').html(result);
            getSortobj();
